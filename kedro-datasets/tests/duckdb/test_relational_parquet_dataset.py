...
from kedro_datasets.duckdb import ParquetRelationalDataSet

FILENAME = "test.parquet"


def assert_query_equal(query_a, query_b):
    """
    Quick utility asserting equality of results yielded by 2 DuckDBPyRelation objects
    """

    assert tuple(zip(query_a.columns, query_a.types)) == tuple(
        zip(query_b.columns, query_b.types)
    ), "Query dimensions do not match"

    with duckdb.connect(":memory:") as conn:

        differing_records = conn.query(
            """
        ...
        assert len(differing_records) == 0
...
@pytest.fixture
def parquet_relational_data_set(filepath_parquet, load_args, save_args, fs_args):
    return ParquetRelationalDataSet(
...
@pytest.fixture
def versioned_parquet_relational_data_set(filepath_parquet, load_version, save_version):
    return ParquetRelationalDataSet(
...
class TestParquetRelationalDataSet:
    def test_credentials_propagated(self, mocker):
        ...
        ParquetRelationalDataSet(filepath=FILENAME, credentials=credentials)
    ...
    def test_save_and_load(self, tmp_path, dummy_dataframe):
        ...
        data_set = ParquetRelationalDataSet(filepath=filepath)
        data_set.save(dummy_dataframe)
        reloaded = data_set.load()

        assert_query_equal(data_set, reloaded)
        # No harm in a second check to make sure things materialize the same as well:
        assert_frame_equal(dummy_dataframe.df(), reloaded.df())
    ...
    def test_exists(self, parquet_relational_data_set, dummy_dataframe):
        """Test `exists` method invocation for both existing and
        nonexistent data set."""
        assert not parquet_relational_data_set.exists()
        parquet_relational_data_set.save(dummy_dataframe)
        assert parquet_relational_data_set.exists()

    @pytest.mark.parametrize(
        "load_args", [{"k1": "v1", "index": "value"}], indirect=True
    )
    def test_load_extra_params(self, parquet_relational_data_set, load_args):
        """Test overriding the default load arguments."""
        for key, value in load_args.items():
            assert parquet_relational_data_set._load_args[key] == value

    @pytest.mark.parametrize(
        "save_args", [{"k1": "v1", "index": "value"}], indirect=True
    )
    def test_save_extra_params(self, parquet_relational_data_set, save_args):
        """Test overriding the default save arguments."""
        for key, value in save_args.items():
            assert parquet_relational_data_set._save_args[key] == value
    ...
    def test_load_missing_file(self, parquet_relational_data_set):
        """Check the error when trying to load missing file."""
        pattern = (
            r"Failed while loading data from data set ParquetRelationalDataSet\(.*\)"
        )
        with pytest.raises(DataSetError, match=pattern):
            parquet_relational_data_set.load()
    ...
    def test_read_partitioned_file(self, mocker, tmp_path, dummy_dataframe):
        ...
        data_set = ParquetRelationalDataSet(filepath=tmp_path.as_posix())
    ...
    def test_write_to_dir(self, dummy_dataframe, tmp_path):
        data_set = ParquetRelationalDataSet(filepath=tmp_path.as_posix())
        pattern = "Saving ParquetRelationalDataSet to a directory is not supported"
    ...
    def test_read_from_non_local_dir(self, mocker):
        ...
        data_set = ParquetRelationalDataSet(filepath="s3://bucket/dir")
    ...
    def test_read_from_file(self, mocker):
        ...
        data_set = ParquetRelationalDataSet(filepath="/tmp/test.parquet")

        data_set.load()
        assert mock_pandas_call.call_count == 1

    def test_arg_partition_cols(self, dummy_dataframe, tmp_path):
        data_set = ParquetRelationalDataSet(
            filepath=(tmp_path / FILENAME).as_posix(),
...
class TestParquetRelationalDataSetVersioned:
    def test_version_str_repr(self, load_version, save_version):
        """Test that version is in string representation of the class instance
        when applicable."""
        ds = ParquetRelationalDataSet(filepath=FILENAME)
        ds_versioned = ParquetRelationalDataSet(
        ...
        assert "ParquetRelationalDataSet" in str(ds_versioned)
        assert "ParquetRelationalDataSet" in str(ds)
        assert "protocol" in str(ds_versioned)
        assert "protocol" in str(ds)

    def test_save_and_load(
        self, versioned_parquet_relational_data_set, dummy_dataframe, mocker
    ):
        ...
        mocker.patch(
            ...
            return_value=PyFileSystem(
                FSSpecHandler(versioned_parquet_relational_data_set._fs)
            ),
        )
        versioned_parquet_relational_data_set.save(dummy_dataframe)
        reloaded_df = versioned_parquet_relational_data_set.load()
        assert_frame_equal(dummy_dataframe, reloaded_df)

    def test_no_versions(self, versioned_parquet_relational_data_set):
        """Check the error if no versions are available for load."""
        pattern = r"Did not find any versions for ParquetRelationalDataSet\(.+\)"
        with pytest.raises(DataSetError, match=pattern):
            versioned_parquet_relational_data_set.load()

    def test_exists(
        self, versioned_parquet_relational_data_set, dummy_dataframe, mocker
    ):
        """Test `exists` method invocation for versioned data set."""
        assert not versioned_parquet_relational_data_set.exists()
        mocker.patch(
            "pyarrow.fs._ensure_filesystem",
            return_value=PyFileSystem(
                FSSpecHandler(versioned_parquet_relational_data_set._fs)
        ...
        versioned_parquet_relational_data_set.save(dummy_dataframe)
        assert versioned_parquet_relational_data_set.exists()
    ...
    def test_prevent_overwrite(
        ...
        versioned_parquet_relational_data_set.save(dummy_dataframe)
        pattern = (
            r"Save path \'.+\' for ParquetRelationalDataSet\(.+\) must "
    ...
    def test_save_version_warning(
        self,
        versioned_parquet_relational_data_set,
        load_version,
        save_version,
        dummy_dataframe,
        mocker,
        ...
        pattern = (
            ...
            rf"'{load_version}' for ParquetRelationalDataSet\(.+\)"
        )
        mocker.patch(
            "pyarrow.fs._ensure_filesystem",
            return_value=PyFileSystem(
    ...
    def test_versioning_existing_dataset(
        ...
        assert parquet_relational_data_set.exists()
        assert (
            parquet_relational_data_set._filepath
            == versioned_parquet_relational_data_set._filepath
        )
        pattern = (
            f"(?=.*file with the same name already exists in the directory)"
            f"(?=.*{versioned_parquet_relational_data_set._filepath.parent.as_posix()})"
        )
        with pytest.raises(DataSetError, match=pattern):
            versioned_parquet_relational_data_set.save(dummy_dataframe)

        # Remove non-versioned dataset and try again
        Path(parquet_relational_data_set._filepath.as_posix()).unlink()
        versioned_parquet_relational_data_set.save(dummy_dataframe)
        assert versioned_parquet_relational_data_set.exists()
